...
public class SelFTestClass {
    ...
    public static void main(String[] args) {
        ...
        String string = "This is pk prajapati ";
        String string1 = Stream.of(string).map(word -> new StringBuffer(word).reverse()).collect(Collectors.joining(" "));
        System.out.println( "Reverse String" + string1);
        String string2 = Arrays.stream(string.split(" ")).map(w-> new StringBuffer(w).reverse()).collect(Collectors.joining(""));
        System.out.println(string2);
        Map<String, Long> string3 = Stream.of(string).collect(Collectors.groupingBy(Function.identity(), Collectors.counting() ))
                .entrySet()
                .stream()
                .collect(Collectors.toMap(Map.Entry::getKey, Map.Entry::getValue));
        String[] words = {"Hello", "world", "this", "is", "a", "StringBuilder", "example"};
        String string4=  Stream.of(words).map(StringBuffer::new).collect(Collectors.joining(" "));
        System.out.println(string4);

        StringBuffer stringBuffer = new StringBuffer(Arrays.toString(words)).reverse();

        System.out.println("stringBuffer string :: "+ stringBuffer);

        List<Integer>  integerList= Arrays.asList(2,5,6,7);

        Optional<Integer> second = integerList.stream().sorted(Comparator.reverseOrder()).skip(2).findFirst();
        second.get();
